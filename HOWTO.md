...
- **Scope it to a place** with **📍 City or country** (`searchLocation`), e.g. `Belgium`, so `Hilton` +
  `Belgium` returns only Belgian Hiltons. A country must be paired with a keyword; a city can be listed on its
  own (no keyword).
